...
class PDF extends FPDF
{
    ...
    function __construct($orientation, $unit, $size, $data, $fecha_formateada)
    {
        ...
        $this->AliasNbPages(); // Alias para el total de páginas
        ...
        $this->customer_doc =
            isset($data[0]['customer_doc']) ? $data[0]['customer_doc'] : '199';
        $this->doc_type =
            isset($data[0]['doc_type']) ? $data[0]['doc_type'] : 'OTROS';
        $this->ai =
            isset($data[0]['id']) ? conca0($data[0]['id']) : '1';
    ...
    {
        global $user;
        $this->SetY(-15); // Ajusta la posición Y del pie de página
        $this->SetFont('times', 'I', 5);
        // Texto centrado: www.lacasaderepuestos.com.pe
        $this->Cell(0, 10, mb_convert_encoding('www.lacasaderepuestos.com.pe', 'ISO-8859-1', 'UTF-8'), 0, 1, 'C');
        // Paginación alineada a la derecha
        $this->SetY(-15); // Ajusta la posición Y del pie de página
        $this->SetX(-30); // Ajusta la posición X a la derecha
        $this->SetFont('Arial', 'I', 8);
        $this->Cell(0, 10, mb_convert_encoding('Página ' . $this->PageNo() . ' de {nb}', 'ISO-8859-1', 'UTF-8'), 0, 0, 'R');
        // Versión y usuario alineados a la izquierda
        $this->SetY(-15); // Ajusta nuevamente la posición Y
        $this->SetFont('Times', 'I', 5);
        $this->Cell(0, 10, mb_convert_encoding('v.' . VERSION . ' - USUARIO: ' . $user['name'], 'ISO-8859-1', 'UTF-8'), 0, 0, 'L');
    }
    ...
    function ProductTable($header, $data)
    {
        ...
        foreach ($data as $row) {
            $this->SetX(15);
            $this->Cell($w[0], 6, $row['qty'], 'LTB', 0, 'C');
            $this->Cell($w[1], 6, mb_convert_encoding($row['product_name'], 'ISO-8859-1', 'UTF-8'), 'LTB', 0, 'L');
            $this->Cell($w[2], 6, number_format($row['price_unit'], 2), 'LTB', 0, 'R');
            $this->Cell($w[3], 6, number_format($row['subtotal'], 2), 'LTRB', 0, 'R');
            $this->Ln();
        }
    }
...
if (isset($_SESSION['post_data'])) {
    ...
    $nombre_archivo = 'BV' . (isset($data[0]['id']) ? conca0($data[0]['id']) : '1') . '-' . $fecha . '.pdf';
